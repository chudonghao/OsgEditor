...
StateSetView::StateSetView(QWidget *parent) : QWidget(parent), ui(new Ui)
{
    ...
    m_textureListModel = new TextureListModel(this);
    //ui->texture_list_view->setModel(m_textureListModel);
    ui->texture_tree_view->setModel(m_textureListModel);
    connect(ui->texture_tree_view, &QTreeView::clicked, this, [=](const QModelIndex &index)
    {
        ...
        int mode_index = 0;
        auto texture = m_textureListModel->getTexture(index);
        auto texgen = m_textureListModel->getTexGen(index);
        m_texture = texture;
        m_texgen = texgen;
        if (texture)
        {
    ...
    auto updateWrapFunc = [=](int index)
    {
        ...
        ref_ptr<Texture> rpTexture = m_texture;
        if (rpTexture)
        {
    ...
    {
        ref_ptr<TexGen> rpTexGen = m_texgen;
        if (rpTexGen)
        {
...
{
    ref_ptr<StateSet> rpStateSet = m_stateSet;
    m_textureListModel->setStateSet(rpStateSet);
    m_texture = nullptr;
    m_texgen = nullptr;
    if (rpStateSet)
    {
